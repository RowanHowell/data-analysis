...
DPmodelfitBasic = function(SDataFile,Screen,compDistPlot = F, fitPlot = F){
  ...
  return(list(Zcutoff=Zcutoff, means=means, pros=pros, sds=sds, Xp = Xp,Xp1 = Xp1, Xb = Xb, XpFDR = XpFDR,Xq5 = Xq5,Xq50 = Xq50, XpV20 = XpV20, XpV40=XpV40,failure = failure))
...
predValidation = function(ValData, Params, Screen, combineplates = F){
  ...
  ValData = arrange(ValData, desc(sLGR)) %>% filter(sLGR>0) # remove enhancers from SPB validations
  ...
  if(combineplates == F){
    FPR = c()
    LGR = c()
    for(k in (BinSize+1):(nrow(ValData)-BinSize)){
      int = ValData[(k-BinSize):(k+BinSize),]
      FPR[k-BinSize] = nrow(filter(int,  Validation == 0))/nrow(int)
      LGR[k-BinSize] = mean(int$sLGR)
    }
    Data2 = tibble(X = LGR,Y = FPR, line = paste0("Screen Data"))
  }
  ...
  png(paste0(Screen,"FPRpred.png"), width = 800)
